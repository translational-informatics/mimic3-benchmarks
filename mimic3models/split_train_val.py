...
data_dir = '/hpcdata1/overby-mimic/asthama_extracted/data/root/'
...
val_patients = set()
with open(data_dir+"mimic3models/valset.csv", "r") as valset_file:
    for line in valset_file:
        x, y = line.split(',')
        if int(y) == 1:
            val_patients.add(x)

with open(data_dir+"%s/train/listfile.csv" % args.task) as listfile:
...
with open(data_dir+"%s/train_listfile.csv" % args.task, "w") as train_listfile:
    train_listfile.write(header)
    for line in train_lines:
        train_listfile.write(line)

with open(data_dir+"%s/val_listfile.csv" % args.task, "w") as val_listfile:
    val_listfile.write(header)
    for line in val_lines:
        val_listfile.write(line)

shutil.copy(data_dir+"%s/test/listfile.csv" % args.task,
            data_dir+"%s/test_listfile.csv" % args.task)
